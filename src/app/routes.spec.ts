...
describe('App Routes', () => {
  ...
  it('should define the login route with a loadComponent function', () => {
    const login = routes.find(r => r.path === 'login') as Route;
  ...
  it('should define the shell route at root with AppShellComponent and children', () => {
    const shell = routes.find(r => r.path === '') as Route;
    expect(shell).toBeTruthy();
    expect(shell.component).toBe(AppShellComponent);
  ...
  describe('shell child routes', () => {
    ...
    it('home child route should match full path and lazy-load', () => {
      const home = children.find(c => c.path === '') as Route;
      expect(home).toBeTruthy();
      expect(home.pathMatch).toBe('full');
      expect(typeof home.loadComponent).toBe('function');
    });

    it.each(['sessions', 'songs', 'metrics'])(
      'child route "%s" should lazy-load a component',
      (p) => {
        const child = children.find(c => c.path === p) as Route;
  ...
  it('should define a wildcard 404 route that lazy-loads', () => {
    const notFound = routes.find(r => r.path === '**') as Route;
